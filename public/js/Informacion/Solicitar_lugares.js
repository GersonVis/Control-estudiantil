var con_lugares
solicitar_lugares = function (url, contenedor_respuesta) {

    this.solicitar_datos = async function solicitar_datos() {
        let respuesta = await enviar_formulario(url)

        if (respuesta.respuesta) {
            con_lugares = respuesta.contenido.map(datos => {//se piden los lugares y se crea un elemento
                // por cada lugar retornado
...
window.addEventListener("load", function (ev) {
   /* let interfaz_lugar = new Lugar({
    ...
    add_eventos_lugar({
        principal: interfaz_lugar.get_elemento_principal(), datos: {
            Id_lugar: "General"
...
*/
    let lugares_solicitud=new solicitar_lugares(url_lugares, lista_contenedor_lugares)//url_alumnos se encuentra en el index
    lugares_solicitud.solicitar_datos()
