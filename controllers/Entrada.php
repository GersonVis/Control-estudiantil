...
class Entrada extends Controller
{
    ...
    function minutosPorEntrada($no_control = "")
    { //no_control por url
    ...
    function archivoConsulta()
    {
        ...
        $nombre_archivo = date("ydhms") . ".csv";
        $creacion = $this->crear_archivo($nombre_archivo);
    
        //consultamos informacion a la base de datos
        $consulta=$this->modelo->descargarconsulta($datos);
        ...
        if (!is_string($creacion)) {
            $resultado = $this->respuesta_formateada(true, "archivo creado", "creacion archivo", $consulta["registros_afectados"], array(0=>array("Nombre" => $nombre_archivo)));
            $this->view->resultado = $resultado;
            $this->view->renderizar();
            exit();
        }
        $resultado=$this->respuesta_formateada(false, $creacion, "creacion archivo", "0");
        $this->view->resultado = $resultado;
        $this->view->renderizar();
    }
